...
Or follow the steps to install python3
```
sudo apt update
...
Now with virtual env installed, we will need to download custom rasa from github and enter into it

Run the following command to do so :

```
git clone https://github.com/athenasaurav/custom_rasa_2_5.git
...
Now before building we need to make sure we have one neccessary tool installed as MAKE,
To check Make installation & To install dependencies and `rasa` itself in editable mode execute

```bash
make install
```

If it returns not installed error for make
Try installing Make using following Command
```
sudo apt-get update
